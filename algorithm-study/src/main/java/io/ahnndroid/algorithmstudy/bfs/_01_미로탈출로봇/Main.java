...
public class Main {
	...
	// 미로의 행렬 사이즈를 입력 받는 전역 변수 (X는 세로(= 열), Y는 가로(= 행))
	static int X, Y;
	...
	// 큐에서 꺼낸 원소 기준 상하좌우 탐색용 오프셋 배열 전역 변수 선언 
	...
	static void initStaticVariables() {
		// 미로판 배열 초기화
		miro = new char[100 + 10][100 + 10];
		
		// 미로 방문 여부 마킹 배열 초기화
		visit = new int[100 + 10][100 + 10];
		
		// 큐 초기화
		Queue = new Element[(100 + 10) * (100 + 10)];
		
		// 큐의 쓰기 & 읽기 위치 저장 포인터 변수 초기화
		wp = 0;
		rp = 0;
		
		// 이동 시간 누적합 출력용 변수 초기화
		sol = 0;
	}
